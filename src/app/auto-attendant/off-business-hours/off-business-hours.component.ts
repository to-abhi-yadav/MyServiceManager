...
@Component({
  selector: 'app-off-business-hours',
  templateUrl: './off-business-hours.component.html',
  styleUrls: ['./off-business-hours.component.scss']
})
export class OffBusinessHoursComponent implements OnInit, AfterViewInit {

  currentUser: User;
  offMenu: Menus;
  offAnnouncement: Announcements;
  hasOffAnnouncement = false;
  isFileUpload = false;
  isTTS = false;
  ttsMessage = "";
  selectedFile: File;
  selectedFileName: any;
  @ViewChild('offBusContent') public offBusContent: ElementRef;
  ...
  getKeyPadHighlights() {
    for (let i in this.option){
      if(this.option[i].action === "no_mapping") {
        this.keyboard3.removeButtonTheme(this.option[i].key_press.toString(), "hg-gold");
      }
      ...
      this.option[i].action === "announce-release" || this.option[i].action === "release") {
        this.keyboard3.addButtonTheme(this.option[i].key_press.toString(), "hg-gold");
      }      
    }
  }
  ...
  ngAfterViewInit() {
    this.keyboard3 = new Keyboard(".simple-keyboard-three", {
  ...
  onInputChange = (event: any) => {
    this.keyboard3.setInput(event.target.value);
    this.value = event.target.value
    this.openLg()
  };

  private openLg() {
    this.modalRef3 = this.modalService.open(this.offBusContent, { centered: true, ariaLabelledBy: 'modal-basic-title'})
    this.modalRef3.result.then((result) => {
      console.log(result)
      this.closeResult = `Closed with: ${result}`;
    }, (reason) => {
      this.value = null
      this.keyboard3.clearInput()
        ...
        for (let i in this.option){
          if(this.option[i].key_press === this.value)
          {
            ...
            if(this.action === "no_mapping") {
              this.option[i].parm = "";
              this.keyboard3.removeButtonTheme(this.value.toString(), "hg-gold")
            }
            else if(this.action === "release") {
              this.option[i].parm = "";
              this.keyboard3.addButtonTheme(this.value.toString(), "hg-gold");
            }
            else if(this.action === "transfer"){
  ...
  saveUploadFile() {
    ...
    console.log(this.offMenu.announcement_id)
    const reader = new FileReader();
    //reader.readAsDataURL(this.selectedFile)
    //console.log(reader.result)
    this.aaService.addAnnouncementFile(this.currentUser.selectedTn.number, this.currentUser.accountId,
      "addannouncement", Number(this.offMenu.announcement_id), this.selectedFile, "announce-release")
      .subscribe(
        (res:'text') => {
  ...
  updateUploadFile() {
    ...
    //console.log()vcx
    //this.getAutoattendantAnnouncements()
    console.log(this.offMenu)
    //reader.readAsDataURL(this.selectedFile)
    //console.log(reader.result)
    this.aaService.updateAnnouncementFile(this.currentUser.selectedTn.number, this.currentUser.accountId,
      "updateannouncement", Number(this.offMenu.announcement_id), this.selectedFile, "announce-release")
      .subscribe(
        (res:'text') => {
          const json = this.aaService.parseXml(res)
          console.log(json)
  ...
  uploadFile() {
    if(this.offMenu.announcement_id === "") {
      this.saveUploadFile()
    }
    else {
      ...
      .subscribe(
        (res: 'text') => {
          ...
          switch (json['Response']['AutoAttendant']['Menu'][0]['name']) {
            case "Off Hours":
              this.offMenu = json['Response']['AutoAttendant']['Menu'][0];
              this.mAnnouncementID = this.offMenu.announcement_id;
              this.menu_id = this.offMenu.menu_id;
          ...
          switch (json['Response']['AutoAttendant']['Menu'][1]['name']) {
            case "Off Hours":
              this.offMenu = json['Response']['AutoAttendant']['Menu'][1];
              this.mAnnouncementID = this.offMenu.announcement_id;
              this.menu_id = this.offMenu.menu_id;
              this.getUploadFile(this.mAnnouncementID);
              this.getAA(this.menu_id);
          ...
          switch (json['Response']['AutoAttendant']['Menu'][2]['name']) {
            case "Off Hours":
              this.offMenu = json['Response']['AutoAttendant']['Menu'][2];
              this.mAnnouncementID = this.offMenu.announcement_id;
              this.menu_id = this.offMenu.menu_id;
              this.getUploadFile(this.mAnnouncementID);
              this.getAA(this.menu_id);
  ...
  getAutoattendantAnnouncements() {
    this.aaService.retrieveAutoattendantAnouncements(this.currentUser.selectedTn.number, this.currentUser.accountId, "announcements", Number(this.offMenu.announcement_id))
      .subscribe(
        (res: 'text') => {
          const json = this.aaService.parseXml(res)
          switch (json['Response']['AutoAttendant']['Announcements']['Announcement'][0]['name']) {
            case "Off Hours":
              this.offAnnouncement = json['Response']['AutoAttendant']['Announcements']['Announcement'][0];
              break;
            default: 
              null
              break;
          ...
          switch (json['Response']['AutoAttendant']['Announcements']['Announcement'][1]['name']) {
            case "Off Hours":
              this.offAnnouncement = json['Response']['AutoAttendant']['Announcements']['Announcement'][1];
              break;
            default: 
              null
              break;
          ...
          switch (json['Response']['AutoAttendant']['Announcements']['Announcement'][2]['name']) {
            case "Off Hours":
              this.offAnnouncement = json['Response']['AutoAttendant']['Announcements']['Announcement'][2];
              break;
            default: 
              null
              break;
          }
          console.log(this.offAnnouncement)
        })
  }
