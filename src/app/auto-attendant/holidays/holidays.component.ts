...
@Component({
  ...
  styleUrls: ['./holidays.component.scss', './number-pad-holidays.scss']
})
export class HolidaysComponent implements OnInit, AfterViewInit {
  // Reference the modal content
  @ViewChild('holidaysContent') public holidaysContent: ElementRef;
  currentUser: User;
  holidayMenu: Menus;
  holidayAnnouncement: Announcements;
  hasHolidayAnnouncement = false;
  isFileUpload = false;
  isTTS = false;
  ttsMessage = "";
  selectedFile: File;
  ...
  getKeyPadHighlights() {
    for (let i in this.option){
      if(this.option[i].action === "no_mapping") {
        this.keyboard4.removeButtonTheme(this.option[i].key_press.toString(), "hg-gold");
      }
      ...
      this.option[i].action === "announce-release" || this.option[i].action === "release") {
        this.keyboard4.addButtonTheme(this.option[i].key_press.toString(), "hg-gold");
      }      
    }
  }
  ...
  ngAfterViewInit() {
    this.keyboard4 = new Keyboard(".simple-keyboard-four", {
  ...
  onInputChange = (event: any) => {
    this.keyboard4.setInput(event.target.value);
    this.value = event.target.value
    this.openLg()
  };

  private openLg() {
    this.modalRef4 = this.modalService.open(this.holidaysContent, { centered: true, ariaLabelledBy: 'modal-basic-title'})
    this.modalRef4.result.then((result) => {
      console.log(result)
      this.closeResult = `Closed with: ${result}`;
    }, (reason) => {
      this.value = null
      this.keyboard4.clearInput()
        ...
        for (let i in this.option){
          if(this.option[i].key_press === this.value)
          {
            ...
            if(this.action === "no_mapping") {
              this.option[i].parm = "";
              this.keyboard4.removeButtonTheme(this.value.toString(), "hg-gold")
            }
            else if(this.action === "release") {
              this.option[i].parm = "";
              this.keyboard4.addButtonTheme(this.value.toString(), "hg-gold");
            }
            else if(this.action === "transfer"){
  ...
  saveUploadFile() {
    ...
    console.log(this.holidayMenu.announcement_id)
    const reader = new FileReader();
    //reader.readAsDataURL(this.selectedFile)
    //console.log(reader.result)
    this.aaService.addAnnouncementFile(this.currentUser.selectedTn.number, this.currentUser.accountId,
      "addannouncement", Number(this.holidayMenu.announcement_id), this.selectedFile, "announce-release")
      .subscribe(
        (res:'text') => {
  ...
  updateUploadFile() {
    ...
    //console.log()vcx
    //this.getAutoattendantAnnouncements()
    console.log(this.holidayMenu)
    //reader.readAsDataURL(this.selectedFile)
    //console.log(reader.result)
    this.aaService.updateAnnouncementFile(this.currentUser.selectedTn.number, this.currentUser.accountId,
      "updateannouncement", Number(this.holidayMenu.announcement_id), this.selectedFile, "announce-release")
      .subscribe(
        (res:'text') => {
          const json = this.aaService.parseXml(res)
          console.log(json)
  ...
  uploadFile() {
    if(this.holidayMenu.announcement_id === "") {
      this.saveUploadFile()
    }
    else {
      ...
      .subscribe(
        (res: 'text') => {
          ...
          switch (json['Response']['AutoAttendant']['Menu'][0]['name']) {
            case "Holidays":
              this.holidayMenu = json['Response']['AutoAttendant']['Menu'][0];
              this.mAnnouncementID = this.holidayMenu.announcement_id;
              this.menu_id = this.holidayMenu.menu_id;
          ...
          switch (json['Response']['AutoAttendant']['Menu'][1]['name']) {
            case "Holidays":
              this.holidayMenu = json['Response']['AutoAttendant']['Menu'][1];
              this.mAnnouncementID = this.holidayMenu.announcement_id;
              this.menu_id = this.holidayMenu.menu_id;
              this.getUploadFile(this.mAnnouncementID);
              this.getAA(this.menu_id);
          ...
          switch (json['Response']['AutoAttendant']['Menu'][2]['name']) {
            case "Holidays":
              this.holidayMenu = json['Response']['AutoAttendant']['Menu'][2];
              this.mAnnouncementID = this.holidayMenu.announcement_id;
              this.menu_id = this.holidayMenu.menu_id;
              this.getUploadFile(this.mAnnouncementID);
              this.getAA(this.menu_id);
  ...
  getAutoattendantAnnouncements() {
    this.aaService.retrieveAutoattendantAnouncements(this.currentUser.selectedTn.number, this.currentUser.accountId, "announcements", Number(this.holidayMenu.announcement_id))
      .subscribe(
        (res: 'text') => {
          const json = this.aaService.parseXml(res)
          switch (json['Response']['AutoAttendant']['Announcements']['Announcement'][0]['name']) {
            case "Holidays":
              this.holidayAnnouncement = json['Response']['AutoAttendant']['Announcements']['Announcement'][0];
              break;
            default: 
              null
              break;
          ...
          switch (json['Response']['AutoAttendant']['Announcements']['Announcement'][1]['name']) {
            case "Holidays":
              this.holidayAnnouncement = json['Response']['AutoAttendant']['Announcements']['Announcement'][1];
              break;
            default: 
              null
              break;
          ...
          switch (json['Response']['AutoAttendant']['Announcements']['Announcement'][2]['name']) {
            case "Holidays":
              this.holidayAnnouncement = json['Response']['AutoAttendant']['Announcements']['Announcement'][2];
              break;
            default: 
              null
              break;
          }
          console.log(this.holidayAnnouncement)
        })
  }
